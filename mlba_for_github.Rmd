...
## function to do graph of simulated choices over time
...
png(file=paste("share_choice_with_first_simul_",treatment,".png",sep=""),width=650, height=550)
...
save_kable(paste("table_clicks_shares_",effect,".tex", sep=""))
...
First_two_choices <- read.csv2("First_five_choices_exp1.csv", sep=",")
...
write.csv(rt,file="rt_exp_1.csv", row.names = FALSE)

# saving menus with a CS and three options

menus_cs_exp1<-rt[rt$type==1&rt$id==626312,][,9:14]
write.csv(menus_cs_exp1,file="menus_cs_exp1.csv", row.names = FALSE)



# load data locally for estimation
rt<-read.csv("rt_exp_1.csv")
...
First_two_choices <- read.csv2("First_two_choices_exp2.csv", sep=",")
...
write.csv(menus_cs_exp2,file="menus_cs_exp2.csv", row.names = FALSE)
...
write.csv(rt,file="rt_exp_2.csv", row.names = FALSE)


# load data locally for estimation
rt<-read.csv("rt_exp_2.csv")
...
fit <- mod$variational(
  ...
  output_basename = paste("fit_gen_",treatment,"_",i,sep = ""),
...
fit$summary()
write.csv(fit$draws(format = "df"),file=paste("post_fit_draws_gen_",treatment,"_",i,".csv",sep=""))
fit$save_latent_dynamics_files(dir = , basename = paste("post_fit_latent_gen_","_",i,treatment,sep=""), timestamp = FALSE, random = FALSE)
}
```
...
fit <- mod_rev2$variational(
  ...
  output_dir = working_dir,
  output_basename = paste("rev_fit_gen_",treatment,"_",i,sep = ""),
  save_latent_dynamics=TRUE,
  algorithm="meanfield",
  tol_rel_obj=0.001,
  elbo_samples = 100)
...
write.csv(fit$draws(format = "df"),file=paste("post_rev_fit_draws_gen_",treatment,"_",i,".csv",sep=""))
fit$save_latent_dynamics_files(dir = , basename = paste("post_rev_fit_latent_gen_","_",i,treatment,sep=""), timestamp = FALSE, random = FALSE)
}
```
...
fit <- mod_rev_mix2$variational(
  ...
  output_samples = ndraws,
  output_dir = working_dir,
  output_basename = paste("rev_mix_fit_gen_",treatment,"_",i,sep = ""),
  save_latent_dynamics=TRUE,
  algorithm="meanfield",
  tol_rel_obj=0.005,
  grad_samples = 1,
...
fit$summary()
write.csv(fit$draws(format = "df"),file=paste("post_rev_mix_fit_draws_gen_",treatment,"_",i,".csv",sep=""))
fit$save_latent_dynamics_files(dir = , basename = paste("post_rev_mix_fit_latent_gen_","_",i,treatment,sep=""), timestamp = FALSE, random = FALSE)
}
...
# choose what type of estimate to look at
...
# then put all draws from all chains together for the treatment and model chosen (here, assuming 20 chains)

for(i in 1:nchains){
draws[[i]]<-read_csv(paste("post",type,"fit_draws_gen_",treatment,"_",i,".csv", sep=""))
}
...
write.csv(fit,file=paste("draws_",type,"_fit_gen_",treatment,".csv",sep=""))
write.csv(summarise_draws(fit),file=paste("summary_",type,"_fit_gen_",treatment,".csv",sep=""))
...
summary_single_fit_barre   <- read_csv("summary___fit_gen_barre.csv")
summary_single_fit_chiffre <- read_csv("summary___fit_gen_chiffre.csv")
summary_single_fit_exp2    <- read_csv("summary___fit_gen_exp2.csv")
summary_rev_fit_barre   <- read_csv("summary__rev__fit_gen_barre.csv")
summary_rev_fit_chiffre <- read_csv("summary__rev__fit_gen_chiffre.csv")
summary_rev_fit_exp2    <- read_csv("summary__rev__fit_gen_exp2.csv")
summary_rev_mix_fit_barre   <- read_csv("summary__rev_mix__fit_gen_barre.csv")
summary_rev_mix_fit_chiffre <- read_csv("summary__rev_mix__fit_gen_chiffre.csv")
summary_rev_mix_fit_exp2    <- read_csv("summary__rev_mix__fit_gen_exp2.csv")
...
x=kable(tab, format = "latex", caption = "Parameter estimates, model with only first choices", booktabs = TRUE, digits=2, escape=F) %>% save_kable("tab_single.tex")
...
x=kable(tab, format = "latex", caption = "Parameter estimates, model with revisions", booktabs = TRUE, digits=2, escape=F) %>% save_kable("tab_rev.tex")
...
x=kable(tab, format = "latex", caption = "Parameter estimates, mixed model with revisions", booktabs = TRUE, digits=2, escape=F) %>% save_kable("tab_mixed.tex")

as.pdf(tab, stem="tab_mix_rev")
```

## extracting individual parameter estimates from mixed estimations
...
## summary tables of individual estimates
...
write.csv(individual_parameter_estimates,file="individual_parameter_estimates.csv", row.names = FALSE)



individual_parameter_estimates<-read.csv("individual_parameter_estimates.csv")
...
p
png(file=paste("individual_parameters_correlations.png",sep=""),width=600, height=350)
p
dev.off()


### Better code from Paolo to get better resolution and consistent plot
individual_parameter_estimates<-read.csv("individual_parameter_estimates.csv") %>% as_tibble()
...
ggsave("individual_parameter_correlation_better.png", width = 16/1.5, height = 12/1.5, 
       units = "in", dpi = 300)
...
# simulated options, a is for attraction effect, s is for similarity, c is for compromise
...
write.csv(rt_test,file="individual_indicators.csv", row.names = FALSE)
...
write.csv(rt_test_mean,file="individual_indicator_estimates.csv", row.names = FALSE)
...
png(file=paste("individual_indicator_correlations_attraction.png",sep=""),width=600, height=350)
p_attraction
dev.off()
...
plotme %>% 
  ...
  ggpairs( 
        ...
        diag = list(continuous = "densityDiag", discrete = "barDiag", na = "naDiag"))+
  hrbrthemes::theme_ipsum()
ggsave("individual_indicator_correlation_attraction_better.png", width = 16/1.5, height = 12/1.5, 
       units = "in", dpi = 300)
...
plotme %>% 
  ...
  ggpairs( 
        ...
        diag = list(continuous = "densityDiag", discrete = "barDiag", na = "naDiag"))+
  hrbrthemes::theme_ipsum()
ggsave("individual_indicator_correlation_similarity_better.png", width = 16/1.5, height = 12/1.5, 
       units = "in", dpi = 300)
...
plotme %>% 
  ...
  ggpairs( 
        ...
        diag = list(continuous = "densityDiag", discrete = "barDiag", na = "naDiag"))+
  hrbrthemes::theme_ipsum()
ggsave("individual_indicator_correlation_compromise_better.png", width = 16/1.5, height = 12/1.5, 
       units = "in", dpi = 300)
```
...
rt_test_mean <- read_csv("individual_indicator_estimates.csv")
...
write.csv(tab,file="individual_indicator_estimates_stats.csv", row.names = FALSE)
...
```{r tidy=TRUE}
library(readr)
individual_indicator_estimates <- read_csv("C:/Users/alescia/Dropbox/Research/CS3/05-Analysis/LMBAR/individual_indicator_estimates.csv")
...
twoclusters <- individual_indicator_estimates %>% drop_na() %>% 
  dplyr::group_by(twocluster,treatment)%>%
  dplyr::summarize(mean_treatment = mean(treatment),
            mean_speed = mean(speed),
            mean_precision = mean(precision),
            mean_bias = mean(bias_a),
            mean_stay = mean(stay),
            N = length(stay))
...
individual_indicator_estimates$Calinsky<-as.factor(individual_indicator_estimates$Calinsky)

ggplot(individual_indicator_estimates, aes(x = Calinsky, y = speed, fill = Calinsky)) + 
...
ggplot(individual_indicator_estimates, aes(x = Calinsky, y = precision, fill = Calinsky)) + 
  stat_boxplot(geom = "errorbar",
               width = 0.25) + 
  geom_boxplot(alpha = 0.8,          # Fill transparency
               colour = "#474747",   # Border color
...
ggplot(individual_indicator_estimates, aes(x = Calinsky, y = bias_a, fill = Calinsky)) + 
  stat_boxplot(geom = "errorbar",
               width = 0.25) + 
  geom_boxplot(alpha = 0.8,          # Fill transparency
               colour = "#474747",   # Border color
...
ggplot(individual_indicator_estimates, aes(x = Calinsky, y = stay, fill = Calinsky)) + 
  stat_boxplot(geom = "errorbar",
               width = 0.25) + 
  geom_boxplot(alpha = 0.8,          # Fill transparency
               colour = "#474747",   # Border color
...
write.csv(fourclusters,file="./tables/fourclusters.csv", row.names = FALSE)


library(kableExtra)
x=kable(fourclusters, format = "latex", caption = "Indicator estimates, by type and treatment", booktabs = TRUE, digits=2, escape=F) %>% save_kable("tables/indicators_by_types.tex")

library(kableExtra)
x=kable(fourclustersC, format = "latex", caption = "Indicator estimates, by type and treatment", booktabs = TRUE, digits=2, escape=F) %>% save_kable("tables/indicators_by_typesC.tex")
...
ff_2 <- melt(individual_indicator_estimates[,c("fourcluster","treatment","speed","precision","bias_a","stay")], id.vars=c("fourcluster","treatment"), na.rm=TRUE)
cast_ff_2<-cast(ff_2, variable ~ fourcluster+treatment, function(x) c(mean(x), quantile(x,c(0.05,0.95))))
...
N<-round(1000000/J,0) # number of draws to take up
selected<-sample(ndraws, N, replace=TRUE)
draws_id <- draws_rev_mix_fit_exp2[selected,c(4:8,14,16)] 
